...
int		main()
{
	...
	std::cout << "waiting for WayOfTheHunter " << std::endl;

	while (procId == 0)
	{
		procId = Utils::FindProcessId(L"WayOfTheHunter-Win64-Shipping.exe");
	}

	std::cout << "Way Of The Hunter Found!" << std::endl;

	Globals->processID = procId;
	Globals->processHandle = OpenProcess(PROCESS_ALL_ACCESS, FALSE, procId);

	Globals->moduleBase = Utils::GetModuleBaseAddress(procId, L"WayOfTheHunter-Win64-Shipping.exe");
	updateInfo();
